...
async function localImport() {
    ...
    try {
        ...
        const existingTournaments = await db.collection('tournaments').countDocuments();
        const existingPlayers = await db.collection('players').countDocuments();
        const existingResults = await db.collection('tournamentresults').countDocuments();
        ...
        if (existingResults > 0) {
            ...
            console.log('   To force reimport, manually drop the tournamentresults collection.');
            return;
        }
        ...
        const finalResults = await db.collection('tournamentresults').countDocuments();
...
async function importChampions(db) {
    ...
    for (const champData of championsData) {
        if (!champData.Format || !champData['BOD#'] || champData.Date === null) continue;

        const tournamentDate = new Date(champData.Date);
        if (isNaN(tournamentDate.getTime()) || tournamentDate > new Date()) continue;

        const existing = await db.collection('tournaments').findOne({ bodNumber: champData['BOD#'] });
        if (existing) continue;
...
async function importTournamentFiles(db) {
    ...
    const files = fs.readdirSync(DATA_DIR).filter(file =>
        ...
        !['All Players.json', 'All Scores.json', 'Champions.json'].includes(file)
    );
