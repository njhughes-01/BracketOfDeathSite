...
class CodebaseVectorServer {
                ...
                tools: [
                    {
                        ...
                        description:
                            "Semantic search over the codebase using a vector database. Use this to find relevant code snippets, functions, or architectural patterns given a natural language query.",
                        inputSchema: {
                            type: "object",
                            properties: {
                                query: {
    ...
    private async initEmbedding() {
        ...
        this.generateEmbedding = async (text: string) => {
    ...
    private createMockTable() {
        const data: any[] = [];
        return {
            add: async (rows: any[]) => { data.push(...rows); },
            search: (queryVector: number[]) => ({
                limit: (n: number) => ({
                    execute: async () => {
    ...
    private async search(query: string, limit: number) {
        ...
        return results.map((r: any) => ({
            ...
            endLine: r.endLine,
            similarity: (r as any).score, // LanceDB might return _distance or score depending on version
            content: r.content
        }));
    }
