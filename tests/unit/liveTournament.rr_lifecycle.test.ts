...
describe('Round Robin Lifecycle', () => {
    ...
    beforeEach(() => {
        ...
        const mockQuery = {
            ...
            then: function (resolve: any) { resolve(tournamentMock); }
        };
        ...
        (Match.find as jest.Mock).mockImplementation((query: any = {}) => {
            ...
            if (query && typeof query === 'object') {
                ...
                if (query.round) results = results.filter(m => m.round === query.round);
            }
            return {
    ...
    describe('Playoff Variant', () => {
        ...
        it('3. Transition to Bracket works', async () => {
            const req = { params: { id: 'rr_tourn' }, body: { action: 'start_bracket' } };
            const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
