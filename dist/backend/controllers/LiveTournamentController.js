...
class LiveTournamentController extends base_1.BaseController {
    ...
    getLiveTournament = this.asyncHandler(async (req, res, next) => {
        try {
            const { id } = req.params;
            const tournament = await Tournament_1.Tournament.findById(id).populate('players', 'name').lean();
            if (!tournament) {
                this.sendError(res, 404, 'Tournament not found');
                return;
            }
            console.log('DEBUG: Fetched tournament', id, 'bracketType:', tournament?.bracketType);
            ...
            const liveTournament = {
                ...
                status: t.status,
                players: t.players?.map((p) => p._id ? p._id.toString() : p.toString()),
    ...
    executeTournamentAction = this.asyncHandler(async (req, res, next) => {
        try {
            console.log('DEBUG: executeTournamentAction called with body:', JSON.stringify(req.body));
            const { id } = req.params;
            ...
            switch (action) {
                ...
                case 'set_round': {
                    ...
                    tournament.managementState = tournament.managementState || {};
                    tournament.managementState.currentRound = target;
            ...
            const liveTournament = {
                ...
                managementState: updatedTournamentObj.managementState || {},
                bracketType: updatedTournamentObj.bracketType,
            };
            const response = {
                success: true,
                data: liveTournament,
    ...
    updateMatch = this.asyncHandler(async (req, res, next) => {
        try {
            ...
            if (processedUpdateData.status) {
                ...
                if (processedUpdateData.status === 'completed' && bothScored && t1Score !== t2Score) {
    ...
    generateMatches = this.asyncHandler(async (req, res, next) => {
        try {
            ...
            if (tournament.bracketType === 'double_elimination' && (isLosers || isGrandFinal)) {
                if (round === 'lbr-round-1') {
                    ...
                    const l1Winners = await this.getBracketAdvancingTeams(tournament, 'lbr-round-1');
                    ...
                    const lsfWinners = await this.getBracketAdvancingTeams(tournament, 'lbr-semifinal');
                    ...
                    const lfWinner = await this.getBracketAdvancingTeams(tournament, 'lbr-final');
    ...
    getCurrentRoundRobinRound(matches) {
        const rrOrder = ['RR_R1', 'RR_R2', 'RR_R3'];
        ...
        for (let i = rrOrder.length - 1; i >= 0; i--) {
            if (matches.some(m => m.round === rrOrder[i]))
                return rrOrder[i];
        }
        return 'RR_R1';
    }
    getCurrentBracketRound(matches) {
        const order = ['quarterfinal', 'semifinal', 'final', 'lbr-round-1', 'lbr-round-2', 'lbr-quarterfinal', 'lbr-semifinal', 'lbr-final', 'grand-final'];
        ...
        for (let i = order.length - 1; i >= 0; i--) {
            if (matches.some(m => m.round === order[i]))
                return order[i];
        }
        return 'quarterfinal';
    }
    ...
    async startRegistration(tournament) {
        try {
            ...
            const preselectedCount = (tournament.players?.length || 0) +
                (tournament.generatedTeams?.reduce((sum, team) => sum + (team.players?.length || 0), 0) || 0);
            const isFullyPreselected = maxPlayers > 0 && preselectedCount >= maxPlayers;
            console.log(`Registration check: max=${maxPlayers}, preselected=${preselectedCount}, full=${isFullyPreselected}`);
    ...
    async setTournamentChampion(tournament) {
        try {
            console.log(`DEBUG: setTournamentChampion called for tournament ${tournament._id}`);
            ...
            if (finalMatch && finalMatch.winner) {
                console.log(`DEBUG: Found final match ${finalMatch._id}, winner: ${finalMatch.winner}`);
                const winningTeam = finalMatch.winner === 'team1' ? finalMatch.team1 : finalMatch.team2;
                console.log(`DEBUG: Winning Team Players: ${JSON.stringify(winningTeam.players)}`);
                tournament.champion = {
                    playerId: winningTeam.players[0],
                    playerName: winningTeam.playerNames.join(' & '),
                };
                console.log(`Tournament champion set: ${tournament.champion.playerName} (${tournament.champion.playerId})`);
            }
            else {
                console.log(`DEBUG: Final match not found or no winner. Match: ${finalMatch?._id} Winner: ${finalMatch?.winner}`);
            }
        }
        catch (error) {
    ...
    async getBracketLosingTeams(tournament, round) {
        const matches = await Match_1.Match.find({ tournamentId: tournament._id, round, status: 'completed' });
        const losers = [];
        matches.forEach(match => {
            if (!match.winner)
                return;
    ...
    async generateLosersMatchesForRound(tournament, teams, round) {
        if (!teams || teams.length < 2)
            return [];
        const highestMatch = await Match_1.Match.findOne({ tournamentId: tournament._id })
            .sort({ matchNumber: -1 })
            .select('matchNumber')
            .lean();
        ...
        for (let i = 0; i < teams.length; i += 2) {
            ...
            matches.push({
                ...
                team1: {
                    players: teams[i].players.map((p) => p.playerId),
                    playerNames: teams[i].players.map((p) => p.playerName),
                    score: 0,
                    seed: teams[i].combinedSeed
                },
                team2: {
                    players: teams[i + 1].players.map((p) => p.playerId),
                    playerNames: teams[i + 1].players.map((p) => p.playerName),
                    score: 0,
        ...
        if (matches.length === 0)
            return [];
        const created = await Match_1.Match.insertMany(matches);
        console.log(`Generated ${created.length} matches for ${round}`);
        return created;
    }
    async advanceDoubleElimination(tournament) {
        const allMatches = await Match_1.Match.find({ tournamentId: tournament._id });
    ...
    async createMatchesForRound(tournament, round) {
        try {
            ...
            if (!teams || teams.length === 0) {
                const seedMap = {};
                const seeds = tournament.generatedSeeds || [];
                ...
                const raw = Array.isArray(tournament.players) ? tournament.players : [];
                const uniqueIds = [];
                const seen = new Set();
                ...
                for (let i = 0; i < playersMeta.length; i += teamSize) {
                    ...
                    synthesizedTeams.push({
                        teamId: `${tournament._id || 'tourn'}-T${synthesizedTeams.length + 1}`,
                        teamName,
                        players: group,
                        combinedSeed,
                        combinedStatistics: {},
    ...
    async updateTeamTournamentResult(tournament, team, match, won) {
        try {
            ...
            if (!result.roundRobinScores) {
                result.roundRobinScores = { round1: 0, round2: 0, round3: 0, rrWon: 0, rrLost: 0, rrPlayed: 0, rrWinPercentage: 0 };
            }
            if (!result.bracketScores) {
                result.bracketScores = { r16Won: 0, r16Lost: 0, qfWon: 0, qfLost: 0, sfWon: 0, sfLost: 0, finalsWon: 0, finalsLost: 0, bracketWon: 0, bracketLost: 0, bracketPlayed: 0 };
            }
            if (!result.totalStats) {
                result.totalStats = { totalWon: 0, totalLost: 0, totalPlayed: 0, winPercentage: 0 };
            }
            if (result.totalStats && result.totalStats.bodFinish === 0) {
                delete result.totalStats.bodFinish;
            }
