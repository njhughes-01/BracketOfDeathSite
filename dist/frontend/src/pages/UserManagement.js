...
const CreateUserForm_1 = __importDefault(require("../components/users/CreateUserForm"));
const UserList_1 = __importDefault(require("../components/users/UserList"));
const Card_1 = __importDefault(require("../components/ui/Card"));
const UserManagement = () => {
    const { user } = (0, AuthContext_1.useAuth)();
    const { canManageUsers } = (0, usePermissions_1.usePermissions)();
    ...
    const loadUsers = async () => {
        try {
            setLoading(true);
            setError('');
            const response = await api_1.default.getUsers();
    ...
    const handleCreateUser = async (userData) => {
        try {
            setLoading(true);
            const response = await api_1.default.createUser(userData);
            if (response.success && response.data) {
    ...
    const handleDeleteUser = async (userToDelete) => {
        ...
        try {
            setLoading(true);
            const response = await api_1.default.deleteUser(userToDelete.id);
            if (response.success) {
    ...
    const handleResetPassword = async (userToReset) => {
        ...
        try {
            setLoading(true);
            const response = await api_1.default.resetUserPassword(userToReset.id, {
                newPassword,
                temporary: true,
            });
            if (response.success) {
    ...
    const handleToggleStatus = async (userToToggle) => {
        ...
        try {
            setLoading(true);
            const response = await api_1.default.updateUser(userToToggle.id, {
                enabled: newStatus,
            });
            if (response.success && response.data) {
    ...
    const handleToggleAdminRole = async (userToToggle) => {
        ...
        try {
            ...
            const response = await api_1.default.updateUserRoles(userToToggle.id, newRoles);
            if (response.success) {
                await loadUsers();
                setError('');
            }
    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8 flex justify-between items-center">
        ...
        <button onClick={() => setShowCreateForm(!showCreateForm)} className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
          {showCreateForm ? 'Cancel' : 'Create User'}
        </button>
      </div>

      {error && (<div className="mb-6 rounded-md bg-red-50 p-4">
          <div className="text-sm text-red-700">{error}</div>
        </div>)}

      {showCreateForm && (<div className="mb-8">
          <CreateUserForm_1.default onSubmit={handleCreateUser} loading={loading} onCancel={() => setShowCreateForm(false)}/>
        </div>)}

      <UserList_1.default users={users} loading={loading} onDeleteUser={handleDeleteUser} onResetPassword={handleResetPassword} onToggleStatus={handleToggleStatus} onToggleAdminRole={handleToggleAdminRole}/>

      {user && (<div className="mt-8">
          <Card_1.default padding="lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <dd className="mt-1">
                  {user.roles.map((role) => (<span key={role} className="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded mr-2">
                      {role}
                    </span>))}
                </dd>
              </div>
