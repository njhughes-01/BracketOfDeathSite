...
const EditableText = ({ value, onSave, placeholder = "Click to edit...", multiline = false, className = "", displayClassName = "", editClassName = "", disabled = false, required = false, maxLength, validator }) => {
    ...
    if (isEditing) {
        ...
        <InputComponent ref={inputRef} type={multiline ? undefined : 'text'} value={editValue} onChange={(e) => setEditValue(e.target.value)} onKeyDown={handleKeyDown} onBlur={handleSave} placeholder={placeholder} maxLength={maxLength} disabled={isLoading} className={`input ${editClassName} ${error ? 'border-red-500 focus:ring-red-500' : ''} ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`} rows={multiline ? 3 : undefined}/>
