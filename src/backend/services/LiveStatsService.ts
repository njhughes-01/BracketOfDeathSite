...
export class LiveStatsService {
  ...
  static async calculateLiveTournamentStats(tournamentId: string): Promise<LiveTournamentStats | null> {
    try {
  ...
  private static async calculateTeamStandings(
    ...
    for (const team of teams) {
      ...
      const eliminated = bracketMatches.some(m => 
        m.status === 'completed' && !this.isTeamWinner(team, m)
      );
      ...
      const performanceGrade = this.calculatePerformanceGrade(
        completedMatches.length > 0 ? wonMatches.length / completedMatches.length : 0,
        pointDifferential,
  ...
  private static isTeamInMatch(teamId: string, match: any): boolean {
    return match.team1?.players?.some((p: any) => p.toString().includes(teamId)) ||
  ...
  private static calculateTeamPoints(team: any, matches: any[]): { pointsScored: number; pointsAllowed: number } {
    let pointsScored = 0;
    let pointsAllowed = 0;
  ...
  private static getTeamAdvancement(team: any, bracketMatches: any[]): string | undefined {
  ...
  private static calculatePerformanceGrade(winPercentage: number, pointDiff: number, gamesPlayed: number): string {
  ...
  private static determineCurrentPhase(matches: any[], tournamentStatus: string): { currentPhase: string; currentRound?: string } {
