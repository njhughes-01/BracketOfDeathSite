...
export class TournamentRegistrationService {
   ...
   */
  static async finalizeRegistration(tournamentId: string): Promise<RegistrationResult> {
    try {
      const tournament = await Tournament.findById(tournamentId).populate('registeredPlayers', 'name');
      if (!tournament) {
        return { success: false, message: 'Tournament not found' };
      }
