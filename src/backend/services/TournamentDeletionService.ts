...
export class TournamentDeletionService {
  ...
  private async validateAndPrepareDeletion(
    operation: DeletionOperation,
  ): Promise<ITournament> {
    ...
    try {
      ...
      const [matchCount, resultCount] = await Promise.all([
        ...
        TournamentResult.countDocuments({
  ...
  private async deleteMatchesWithCompensation(
    operation: DeletionOperation,
  ): Promise<void> {
  ...
  private async deleteResultsWithCompensation(
    operation: DeletionOperation,
  ): Promise<void> {
  ...
  private async deleteTournamentEntity(
    operation: DeletionOperation,
  ): Promise<void> {
  ...
  private async attemptCompensation(
    ...
    try {
      ...
      if (
        tournamentStep.status === "completed" &&
        tournamentStep.compensationData?.tournament
      ) {
        try {
  ...
  public isRetryable(error: TournamentDeletionError): boolean {
    return (
      error.retryable &&
      (error.code === "MATCHES_DELETION_FAILED" ||
