...
describe('MailjetService', () => {
    ...
    it('should log email when not configured (development/test)', async () => {
        ...
        const result = await mailjetService.sendClaimInvitation('test@example.com', 'token123', 'John Doe');
    ...
    it('should handle API errors', async () => {
        ...
        await expect(mailjetService.sendClaimInvitation('test@example.com', 'token', 'John')).rejects.toThrow('Failed to send email');
    });
});
