...
describe("SystemController", () => {
  ...
  describe("claimSuperadmin", () => {
    it("should allow claiming if system is uninitialized", async () => {
      // Mock empty superadmin list
      (keycloakAdminService.getUsersInRole as jest.Mock).mockResolvedValue([]);
      (keycloakAdminService.setUserRoles as jest.Mock).mockResolvedValue(
        undefined,
      );
      ...
      expect(keycloakAdminService.setUserRoles).toHaveBeenCalledWith(
        "user-123",
    ...
    it("should handle errors gracefully", async () => {
      // Mock error
      (keycloakAdminService.getUsersInRole as jest.Mock).mockRejectedValue(
