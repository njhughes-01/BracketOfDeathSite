...
describe('PlayerController', () => {
    ...
    describe('getStats', () => {
        it('should return aggregated stats', async () => {
            ...
            await playerController.getStats(mockReq as Request, mockRes as Response, nextMock);
    ...
    describe('getChampions', () => {
        it('should return champions list', async () => {
            ...
            await playerController.getChampions(mockReq as Request, mockRes as Response, nextMock);

            expect(Player.find).toHaveBeenCalledWith({ totalChampionships: { $gte: 2 } });
            expect(jsonMock).toHaveBeenCalledWith(expect.objectContaining({
                success: true,
                data: mockChampions
            }));
    ...
    describe('create (validation)', () => {
        it('should validate games won <= games played', async () => {
            ...
            await playerController.create(mockReq as RequestWithAuth, mockRes as Response, nextMock);

            expect(statusMock).toHaveBeenCalledWith(400);
            expect(jsonMock).toHaveBeenCalledWith(expect.objectContaining({
                error: expect.stringContaining('Games won cannot exceed games played')
            }));
        });
        ...
        it('should validate winning percentage', async () => {
            ...
            expect(statusMock).toHaveBeenCalledWith(400);
            expect(jsonMock).toHaveBeenCalledWith(expect.objectContaining({
                error: expect.stringContaining('Winning percentage must be between 0 and 1')
            }));
        });
    ...
    describe('update (validation)', () => {
        it('should validate best result vs avg finish', async () => {
            ...
            await playerController.update(mockReq as RequestWithAuth, mockRes as Response, nextMock);

            expect(statusMock).toHaveBeenCalledWith(400);
            expect(jsonMock).toHaveBeenCalledWith(expect.objectContaining({
                error: expect.stringContaining('Best result cannot be worse than average finish')
            }));
        });
    });
});
