...
export class TournamentResultController extends BaseController<ITournamentResult> {
  ...
  protected override buildFilter(query: any): ITournamentResultFilter {
    ...
    if (filterParams.playerIds) {
      ...
      filter.players = {
        $in: playerIds.map((id: string) => new Types.ObjectId(id)),
      };
    }
    ...
    if (filterParams.finalRank) {
      ...
      if (filterParams.finalRank_min) {
        (filter["totalStats.finalRank"] as any).$gte = parseInt(
          filterParams.finalRank_min,
        );
      }
      if (filterParams.finalRank_max) {
        (filter["totalStats.finalRank"] as any).$lte = parseInt(
          filterParams.finalRank_max,
        );
      }
    }
    ...
    if (filterParams.bodFinish) {
      ...
      if (filterParams.bodFinish_min) {
        (filter["totalStats.bodFinish"] as any).$gte = parseInt(
          filterParams.bodFinish_min,
        );
      }
      if (filterParams.bodFinish_max) {
        (filter["totalStats.bodFinish"] as any).$lte = parseInt(
          filterParams.bodFinish_max,
        );
      }
    }
    ...
    if (filterParams.winPercentage_min || filterParams.winPercentage_max) {
      ...
      if (filterParams.winPercentage_min) {
        (filter["totalStats.winPercentage"] as any).$gte = parseFloat(
          filterParams.winPercentage_min,
        );
      }
      if (filterParams.winPercentage_max) {
        (filter["totalStats.winPercentage"] as any).$lte = parseFloat(
          filterParams.winPercentage_max,
        );
      }
    }
  ...
  private calculatePlayerStats(results: ITournamentResult[]): any {
    ...
    const totalWins = results.reduce(
      (sum, result) => sum + result.totalStats.totalWon,
      0,
    );
    const totalLosses = results.reduce(
      (sum, result) => sum + result.totalStats.totalLost,
      0,
    );
    const totalGames = results.reduce(
      (sum, result) => sum + result.totalStats.totalPlayed,
      0,
    );
