...
export class LiveTournamentController extends BaseController<ITournament> {
  ...
  getLiveTournament = this.asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const tournament = await Tournament.findById(id).populate('players', 'name').lean();
      ...
      console.log('DEBUG: Fetched tournament', id, 'bracketType:', (tournament as any)?.bracketType);
      ...
      const liveTournament: LiveTournament = {
        ...
        status: t.status,
        players: t.players?.map((p: any) => p._id ? p._id.toString() : p.toString()),
