...
vi.mock("../../components/tournament/LiveStats", () => ({
  default: () => <div data-testid="live-stats">Mock Live Stats</div>,
}));
vi.mock("../../components/tournament/MatchScoring", () => ({
  default: () => <div data-testid="match-scoring">Mock Match Scoring</div>,
}));
vi.mock("../../components/tournament/PlayerLeaderboard", () => ({
  default: () => <div data-testid="player-leaderboard">Mock Leaderboard</div>,
}));
...
describe("TournamentManage Page", () => {
  ...
  beforeEach(() => {
    ...
    apiClient.getLiveTournament = vi
      .fn()
      .mockResolvedValue({ success: true, data: mockTournament });
    apiClient.getTournamentMatches = vi
      .fn()
  ...
  it('shows "Close Reg" and "Start Check-In" in registration phase', async () => {
    ...
    apiClient.getLiveTournament = vi
      .fn()
      .mockResolvedValue({ success: true, data: regTournament });
  ...
  it("calls executeTournamentAction when action button is clicked", async () => {
    apiClient.executeTournamentAction = vi
      .fn()
      .mockResolvedValue({ success: true, data: mockTournament });
    ...
    expect(apiClient.executeTournamentAction).toHaveBeenCalledWith("t1", {
  ...
  it("renders matches grid in bracket phase", async () => {
    ...
    apiClient.getLiveTournament = vi
      .fn()
      .mockResolvedValue({ success: true, data: bracketTournament });
    ...
    apiClient.getTournamentMatches = vi
      .fn()
      .mockResolvedValue({ success: true, data: mockMatches });
