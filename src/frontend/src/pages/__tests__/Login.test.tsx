...
describe('Login Page', () => {
    ...
    it('handles direct login form submission success', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith(
                expect.stringContaining('/auth/realms/bracketofdeathsite/protocol/openid-connect/token'),
    ...
    it('handles direct login validation failure', async () => {
        (global.fetch as any).mockResolvedValue({
            ok: false,
            text: async () => JSON.stringify({ error_description: 'Invalid credentials' })
        });
    ...
    it('toggles password visibility', () => {
        ...
        expect(screen.getByRole('button', { name: /visibility_off/i })).toBeInTheDocument();
