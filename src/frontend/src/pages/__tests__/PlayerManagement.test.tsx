...
describe('Player Management', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        (useAuth as any).mockReturnValue({ isAuthenticated: true, user: { roles: ['admin'] } });
    });
    ...
    describe('PlayerCreate', () => {
        ...
        it('submits new player', async () => {
            ...
            fireEvent.change(screen.getByPlaceholderText(/John Doe/i), { target: { value: 'New Player' } });
    ...
    describe('PlayerEdit', () => {
        ...
        beforeEach(() => {
            (apiClient.getPlayer as any).mockResolvedValue({ success: true, data: mockPlayer });
        ...
        it('updates player data', async () => {
            ...
            fireEvent.change(screen.getByDisplayValue('Existing Player'), { target: { value: 'Updated Name' } });
            fireEvent.click(screen.getByText('Save Changes'));

            await waitFor(() => {
                expect(apiClient.updatePlayer).toHaveBeenCalledWith('123', expect.objectContaining({ name: 'Updated Name' }));
