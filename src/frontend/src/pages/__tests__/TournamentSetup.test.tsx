...
describe("TournamentSetup Page", () => {
  beforeEach(() => {
    ...
    (apiClient.getNextBodNumber as any).mockResolvedValue({
      success: true,
      data: { nextBodNumber: 42 },
    });
    (apiClient.getPlayers as any).mockResolvedValue({
      success: true,
      data: mockPlayers,
    });
    (apiClient.generatePlayerSeeds as any).mockResolvedValue({
      success: true,
      data: [],
    });
    (apiClient.generateTeams as any).mockResolvedValue({
      success: true,
      data: [],
    });
    (apiClient.setupTournament as any).mockResolvedValue({
      success: true,
  ...
  it("completes the full flow and creates tournament", async () => {
    ...
    const dateInput = screen.getByTestId("date-input");
    fireEvent.change(dateInput, { target: { value: "2025-12-25" } });
    fireEvent.change(screen.getByPlaceholderText("Tournament location"), {
      target: { value: "Test Arena" },
    });
    fireEvent.click(nextBtn);
    ...
    // Select a player
    const playerCard1 = screen
      .getByText("Player One")
      .closest('div[class*="border rounded-xl cursor-pointer"]');
    ...
    await waitFor(() => {
      expect(apiClient.setupTournament).toHaveBeenCalledWith(
