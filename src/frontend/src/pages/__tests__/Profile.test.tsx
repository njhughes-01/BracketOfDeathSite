...
vi.mock("../../components/auth/ChangePasswordModal", () => ({
  default: () => (
    <div data-testid="change-password-modal">Mock Change Password Modal</div>
  ),
}));
...
describe("Profile Page", () => {
  ...
  it("does NOT display email verification banner when email IS verified", () => {
    ...
    expect(
      screen.queryByText("Verify your email address"),
  ...
  it('calls sendVerificationEmail when "Verify Now" is clicked', async () => {
    ...
    (apiClient.sendVerificationEmail as any).mockResolvedValue({
      success: true,
    });
