            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                {tournaments.length === 0 && !loading && !error && (
                    <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/10">
                        ...
                        <p className="text-xl text-slate-400">No open tournaments at the moment.</p>
                        <p className="text-slate-500 mt-2">Check back later for upcoming events!</p>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {tournaments.map((tournament) => {
                        ...
                        return (
                            <div key={tournament._id} className="group relative bg-[#1c2230] border border-white/5 rounded-3xl overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-[0_0_30px_rgba(34,197,94,0.1)]">
                                {/* Status Bar */}
                                <div className={`h-1.5 w-full ${isFull ? 'bg-amber-500' : 'bg-primary'}`}></div>
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="space-y-3 mb-8">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className={`font-bold ${isFull ? 'text-amber-500' : 'text-white'}`}>
                                                {registered} / {max}
                                            </span>
                                        </div>
                                        {/* Progress Bar */}
                                        <div className="h-2 bg-black/40 rounded-full overflow-hidden">
                                            <div
                                                ...
                                                style={{ width: `${Math.min((registered / max) * 100, 100)}%` }}
                                            ></div>
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-4 rounded-xl font-bold text-base transition-all flex items-center justify-center gap-2 ${isFull
