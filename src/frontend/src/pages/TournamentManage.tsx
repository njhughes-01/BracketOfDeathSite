...
const TournamentManage: React.FC = () => {
  ...
  const loadTournamentData = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (nextRound) {
          const matchesResponse = await apiClient.getTournamentMatches(
            id,
            nextRound,
          );
  ...
  const getPhaseDisplayInfo = (phase: string) => {
    const phaseMap = {
      ...
      round_robin: {
        label: "Round Robin",
        color: "bg-orange-500/20 text-orange-400 border border-orange-500/50",
        icon: "sync",
      },
      bracket: {
        label: "Bracket Play",
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {liveTournament.phase.phase === "registration" && (
                <>
                  <button
                    onClick={() =>
                      executeTournamentAction({ action: "close_registration" })
                    }
                    className="px-4 py-2 rounded-lg bg-orange-500/20 text-orange-500 font-bold hover:bg-orange-500/30 text-sm"
                    disabled={loading}
                  >
                    Close Reg
                  </button>
              ...
              {liveTournament.phase.phase === "check_in" && (
                <button
                  ...
                  disabled={loading}
                >
                  {isRoundRobin(liveTournament.bracketType)
                    ? "Start Round Robin"
                    : "Start Bracket"}
                </button>
              )}
              ...
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      "Are you sure you want to reset? This is destructive.",
                    )
                  )
          ...
          <div className="bg-surface-dark rounded-2xl border border-white/5 p-5 shadow-lg">
            ...
            <MatchesToolbar
              ...
              onToggleCompact={(v) => {
                setCompactListView(v);
                persistToggle("compact", v);
              }}
              requirePerPlayerScores={requirePerPlayerScores}
              onToggleRequirePerPlayer={(v) => {
                setRequirePerPlayerScores(v);
                persistToggle("requirePerPlayerScores", v);
              }}
              strictTotals={strictTotals}
              onToggleStrictTotals={(v) => {
                setStrictTotals(v);
                persistToggle("strictTotals", v);
              }}
              canConfirmAll={
                isAdmin &&
            ...
            {/* Grid */}
            <div
              className={`mt-4 grid gap-3 ${compactListView ? "grid-cols-1" : "grid-cols-1 xl:grid-cols-2"}`}
            >
