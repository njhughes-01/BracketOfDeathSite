  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 0: // Basic Information
        return (
          <div className="bg-[#1c2230] p-6 rounded-2xl border border-white/5 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-lg font-bold text-white mb-4">Tournament Details</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">BOD Number</label>
                <input
                  type="number"
                  ...
                  min="1"
                />
                <p className="text-[10px] text-slate-500 mt-1">Auto-generated as #{nextBodNumber} (can be overridden)</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Max Players</label>
                <select
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Advancement Criteria</label>
              <textarea
      ...
      case 1: // Player Selection
        return (
          <div className="bg-[#1c2230] p-6 rounded-2xl border border-white/5 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-lg font-bold text-white mb-2">Select Tournament Players</h2>
            <div className="flex items-center justify-between bg-background-dark p-4 rounded-xl border border-white/5">
              <div>
                <p className="text-sm text-white font-medium">
                  Select <span className="text-primary font-bold">{setupData.maxPlayers}</span> players
                </p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className={`size-5 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-primary border-primary' : 'border-slate-600 bg-transparent'}`}>
                        {isSelected && <span className="material-symbols-outlined text-black text-[16px] font-bold">check</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`font-bold text-sm truncate ${isSelected ? 'text-primary' : 'text-white'}`}>{player.name}</p>
                        <div className="text-[10px] text-slate-500 flex items-center gap-2">
                          <span>Matches: {player.gamesPlayed || 0}</span>
                          <span>WR: {((player.winningPercentage || 0) * 100).toFixed(0)}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        return (
          <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-[#1c2230] p-6 rounded-2xl border border-white/5 space-y-4">
              <h2 className="text-lg font-bold text-white mb-2">Seeding Configuration</h2>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Seeding Method</label>
                <select
              ...
              {setupData.seedingConfig.method === 'historical' && (
                <div className="bg-background-dark p-4 rounded-xl border border-white/5 grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 mb-2">Championship Weight</label>
                    <input
                      type="range"
                      min="0"
                      max="1"
                    ...
                    />
                    <span className="text-xs text-primary font-mono">{((setupData.seedingConfig.parameters?.championshipWeight || 0.3) * 100).toFixed(0)}%</span>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 mb-2">Win Percentage Weight</label>
                    <input
                      type="range"
                      min="0"
                      max="1"
                      ...
                      className="w-full accent-primary"
                    />
                    <span className="text-xs text-primary font-mono">{((setupData.seedingConfig.parameters?.winPercentageWeight || 0.4) * 100).toFixed(0)}%</span>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 mb-2">Avg Finish Weight</label>
                    <input
                      type="range"
                      min="0"
                      max="1"
                      ...
                      className="w-full accent-primary"
                    />
                    <span className="text-xs text-primary font-mono">{((setupData.seedingConfig.parameters?.avgFinishWeight || 0.3) * 100).toFixed(0)}%</span>
                  </div>
                </div>
              )}
              ...
              {generatedSeeds.length > 0 && (
                <div className="mt-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Generated Seeds</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-40 overflow-y-auto custom-scrollbar">
                    {generatedSeeds.map(seed => (
                      <div key={`seed-${seed.playerId}`} className="flex items-center justify-between p-3 bg-background-dark/50 border border-white/5 rounded-lg">
                        <span className="text-sm font-bold text-white"><span className="text-primary mr-2">#{seed.seed}</span> {seed.playerName}</span>
                        <span className="text-[10px] text-slate-500">
                          {(seed.statistics.winningPercentage * 100).toFixed(0)}% WR
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-[#1c2230] p-6 rounded-2xl border border-white/5 space-y-4">
              <h2 className="text-lg font-bold text-white mb-2">Team Formation</h2>
              <div>
              ...
              {setupData.teamFormationConfig.method === 'statistical_pairing' && (
                <div className="space-y-3 bg-background-dark p-4 rounded-xl border border-white/5">
                  <label className="flex items-center space-x-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-white">Balance team skill levels</span>
                  </label>
                  <label className="flex items-center space-x-3 cursor-pointer">
                    <input
                      type="checkbox"
                      ...
                      className="size-5 rounded border-slate-600 bg-transparent text-primary focus:ring-primary"
                    />
                    <span className="text-sm text-white">Avoid recent tournament partners</span>
                  </label>
                </div>
              )}
              ...
              {generatedTeams.length > 0 && (
                <div className="mt-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Generated Teams</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-40 overflow-y-auto custom-scrollbar">
                    {generatedTeams.map(team => (
                      <div key={`team-${team.teamId}`} className="p-3 bg-background-dark border border-white/10 rounded-lg">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-bold text-sm text-white"><span className="text-accent mr-1">#{team.combinedSeed}</span> {team.teamName}</span>
                          <span className="text-[10px] text-slate-500">{(team.combinedStatistics?.combinedWinPercentage || 0).toFixed(0)}% Combo</span>
                        </div>
      ...
      case 3: // Tournament Settings
        return (
          <div className="bg-[#1c2230] p-6 rounded-2xl border border-white/5 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-lg font-bold text-white mb-2">Tournament Settings</h2>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Bracket Type</label>
              <select
      ...
      case 4: // Review & Create
        return (
          <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-[#1c2230] p-6 rounded-2xl border border-white/5 space-y-4">
              <h2 className="text-lg font-bold text-white mb-2">Tournament Summary</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Basic Info</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Date</span>
                      <span className="text-white text-sm">{new Date(setupData.basicInfo.date).toLocaleDateString(undefined, { timeZone: 'UTC' })}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">BOD</span>
                      <span className="text-white text-sm">#{setupData.basicInfo.bodNumber}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Format</span>
                      <span className="text-white text-sm">{setupData.basicInfo.format}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Players</span>
                      <span className="text-white text-sm">{setupData.maxPlayers}</span>
                    </div>
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Configuration</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Seeding</span>
                      <span className="text-white text-sm capitalize">{setupData.seedingConfig.method.replace('_', ' ')}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Teams</span>
                      <span className="text-white text-sm capitalize">{setupData.teamFormationConfig.method.replace('_', ' ')}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Bracket</span>
                      <span className="text-white text-sm capitalize">{setupData.bracketType.replace('_', ' ')}</span>
                    </div>
                    <div className="flex justify-between border-b border-white/5 pb-1">
                      <span className="text-slate-400 text-sm">Selected</span>
                      <span className="text-white text-sm">{selectedPlayers.length} Players</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <h4 className="font-bold text-yellow-500 text-sm">Ready to Create</h4>
                <p className="text-xs text-yellow-500/80 mt-1">
                  Once created, the tournament will be set to <strong>{setupData.basicInfo.status}</strong>.
                  {setupData.basicInfo.status === 'open' ? ' Players can immediately start registering.' : ' You can further manage it in the admin dashboard.'}
                </p>
              </div>
            </div>
          </div>
  ...
  if (loading && currentStep === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-background-dark">
        ...
        <span className="mt-4 text-slate-500 animate-pulse">Initializing Setup...</span>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-[#1c2230] rounded-2xl p-4 border border-white/5 overflow-x-auto no-scrollbar">
          <div className="flex items-center min-w-[300px]">
            {steps.map((step, index) => (
              <div key={step.id} className="flex items-center flex-1 last:flex-none">
                <div className={`relative z-10 size-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${index <= currentStep ? 'bg-primary text-black shadow-lg shadow-primary/20' : 'bg-white/10 text-slate-500'}`}>
                  {index < currentStep ? <span className="material-symbols-outlined text-[16px] font-bold">check</span> : index + 1}
                </div>
                {index < steps.length - 1 && (
                  <div className={`flex-1 h-1 mx-2 rounded-full transition-all duration-500 ${index < currentStep ? 'bg-primary' : 'bg-white/5'}`}></div>
                )}
              </div>
            ))}
          </div>
          <div className="flex justify-between mt-2 min-w-[300px]">
            {steps.map((step, index) => (
              <span key={step.id} className={`text-[10px] uppercase font-bold transition-colors ${index === currentStep ? 'text-primary' : 'text-slate-600'}`}>
                {step.title}
              </span>
            ))}
          </div>
            ...
            {currentStep < steps.length - 1 ? (
              <button
                onClick={() => {
                  // Validation
                  if (currentStep === 0) {
                    if (!setupData.basicInfo.date) { setError('Date is required'); return; }
                    if (!setupData.basicInfo.format) { setError('Format is required'); return; }
                    if (!setupData.basicInfo.location) { setError('Location is required'); return; }
                    if (!setupData.basicInfo.bodNumber || setupData.basicInfo.bodNumber <= 0) { setError('Valid BOD Number is required'); return; }
                  }
