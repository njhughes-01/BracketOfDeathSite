    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (searchQuery.trim().length >= 2) {
                setIsSearching(true);
                try {
                    const response = await apiClient.searchPlayers(searchQuery, { limit: 5 });
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 bg-background-dark">
                <div className="size-24 rounded-full bg-[#1c2230] border border-white/5 flex items-center justify-center mb-6 shadow-xl">
                    <span className="material-symbols-outlined text-slate-600 text-5xl">person_off</span>
                </div>
                <h2 className="text-2xl font-bold text-white mb-3">Not Signed In</h2>
                <p className="text-slate-400 mb-8 max-w-sm mx-auto leading-relaxed">
                    Please sign in to view your profile and manage your tournament settings.
                </p>
                <Link to="/login" className="px-8 py-3 bg-primary text-black font-bold rounded-xl shadow-[0_0_20px_rgba(204,255,0,0.3)] hover:shadow-[0_0_30px_rgba(204,255,0,0.5)] hover:bg-white transition-all transform hover:scale-105 active:scale-95">
                    Sign In
                </Link>
            </div>
        );
                    ...
                    <div className="relative p-8 md:p-10 flex flex-col md:flex-row items-center md:items-start gap-8">
                        ...
                        <div className="flex-1 text-center md:text-left space-y-2 pt-2">
                            ...
                            <div className="flex flex-wrap gap-2 justify-center md:justify-start mt-4">
                                {user.roles.map(role => (
                                    <span key={role} className="px-3 py-1 rounded-lg bg-black/40 border border-white/10 text-xs font-bold text-primary uppercase tracking-wider">
                                        {role}
                                    </span>
                                ))}
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-[#1c2230] rounded-3xl border border-white/5 p-6 space-y-4 hover:border-white/10 transition-colors flex flex-col h-full">
                        <div className="flex items-center justify-between">
                            ...
                            {user.playerId && (
                                <Link to={`/players/${user.playerId}`} className="text-xs bg-white/5 hover:bg-white/10 text-primary px-3 py-1 rounded-lg border border-primary/20 transition-colors">
                                    View Full Profile
                                </Link>
                            )}
                        </div>
                            ...
                            {user.playerId ? (
                                ...
                                <div className="py-8 text-center space-y-4">
                                    <div className="size-16 rounded-full bg-white/5 mx-auto flex items-center justify-center">
                                        <span className="material-symbols-outlined text-slate-600 text-3xl">link_off</span>
                                    </div>
                                    <p className="text-slate-400 text-sm max-w-xs mx-auto">
                                        Link your account to a player profile to see your personal statistics here.
                                    </p>
                                    <button
                        ...
                        <div className="relative bg-[#1c2230] rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 border border-white/5">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm">Access system management tools and user controls</p>
                                </div>
                            </div>
                            <Link to="/admin" className="w-full sm:w-auto px-6 py-3 bg-primary text-black font-bold rounded-xl hover:bg-white hover:scale-105 shadow-lg shadow-primary/20 flex items-center justify-center gap-2 transition-all">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="w-full max-w-md bg-[#1c2230] border border-white/10 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-white">Link Player Profile</h3>
                            <button
                        ...
                        <div className="space-y-4">
                            <p className="text-sm text-slate-400">
                                Search for your player name to link it to your account. This allows you to track your stats.
                            </p>
                                    ...
                                    <div className="space-y-1">
                                        {searchResults.map((player) => (
                                            ...
                                            >
                                                ...
                                                <span className="material-symbols-outlined text-slate-600 group-hover:text-primary opacity-0 group-hover:opacity-100 transition-all">link</span>
                                            </button>
                                        ))}
                                    </div>
                                ) : searchQuery.length >= 2 ? (
                                    <div className="flex flex-col items-center justify-center h-32 text-slate-500">
                                        <span className="material-symbols-outlined text-3xl mb-2 opacity-50">person_search</span>
                                        No players found
                                    </div>
                                ) : (
                                    <div className="flex flex-col items-center justify-center h-32 text-slate-500">
                                        <span className="material-symbols-outlined text-3xl mb-2 opacity-50">keyboard</span>
                                        Type to search...
                                    </div>
                                )}
                            </div>
                        ...
                        {isLinking && (
                            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm rounded-2xl flex items-center justify-center z-10 flex-col gap-3">
                                ...
                                <span className="text-white font-bold text-sm">Linking Profile...</span>
                            </div>
                        )}
                    </div>
                </div>
