...
vi.mock('../../utils/tennisValidation', () => ({
  validateTennisScore: vi.fn((s1, s2) => {
    ...
    if ((s1 === 6 && s2 <= 4) || (s1 <= 4 && s2 === 6)) return { isValid: true };
    if ((s1 === 7 && (s2 === 5 || s2 === 6)) || ((s1 === 5 || s1 === 6) && s2 === 7)) return { isValid: true };
    if (s1 === 5 && s2 === 3) return { isValid: false, reason: 'Invalid set score' }; // Specific invalid case
...
describe('MatchScoring Component', () => {
  ...
  it('allows manual save without completion', () => {
    ...
    expect(mockUpdateMatch).toHaveBeenCalledWith(expect.objectContaining({
      matchId: 'm1',
      status: 'in-progress'
    }));
  });
  ...
  it('shows admin override dialog for invalid score (5-3)', async () => {
    ...
    const reasonInput = screen.getByPlaceholderText('e.g., Retire due to injury');
