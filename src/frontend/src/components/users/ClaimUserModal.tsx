...
const ClaimUserModal: React.FC<ClaimUserModalProps> = ({ onClose, onSuccess }) => {
                ...
                <div className="bg-[#1c2230] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
                    <div className="text-center py-6">
                        ...
                        <p className="text-slate-400 mb-6">
                            An email has been sent to <strong>{email}</strong> with instructions to claim the profile for <strong>{selectedPlayer?.name}</strong>.
                        </p>
                        <button
                            onClick={onSuccess}
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
            <div className="bg-[#1c2230] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
                ...
                <p className="text-slate-400 mb-6 text-sm">Invite a user to claim an existing player profile.</p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-bold text-slate-400 ml-1">SELECT PLAYER</label>
                        {selectedPlayer ? (
                    ...
                    >
                        {loading ? <LoadingSpinner size="sm" color="black" /> : (
                            <>
                                <span className="material-symbols-outlined">send</span> Send Invitation
                            </>
                        )}
                    </button>
                </form>
